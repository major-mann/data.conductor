...
var d;if(!h.l)if(m.w()===e.all){h.l=!0;h.cancel.resolve();for(d=0;d<h.c.length;d++)f(h.c[d],h.c[d].index===a?"cancelled":!1);w.reject("cancelled")}else if(m.w()===e.ha)d=h.c.filter(b),1===d.length&&(h.l=!0,h.cancel.resolve(),w.reject("cancelled")),f(h.c[a],"cancelled");else throw Error('unrecognised batch cancel mode "'+m.w()+'"');}function f(a,b){b?(a.l=!0,a.loaded.reject(b)):(a.g=!1,a.l=!1,g.u.qb(a),m.P()===y.Ha?m.ga(!0):m.ga());delete g.g[Math.floor(a.index/a.count)]}var d,k,h,w;if(!g.I)throw Error("adapter has not been set. You must set an adapter before calling execute.");
for(h=C();null===h&&g.u.count();)h=C();w=n.defer();if(h){for(d=0;d<h.c.length;d++)k=Math.floor(h.c[d].index/h.c[d].count),g.g[k]=h.c[d],h.c[d].g=!0,h.c[d].Ra&&h.c[d].Ra.then(c.bind(null,h.c[d].index));h.cancel=n.defer();d=g.I.find(g.filter,{Hb:h.index,Eb:h.count,cancel:h.cancel.H});d.then(a).catch(b)}else w.resolve([]);return w.H}function C(){function a(b,c){b.index===c.index?b.count=Math.max(b.count,c.count):(b.count+=c.count,c.index<b.index&&(b.index-=c.count));b.c.push(c)}function b(a,c){var d=
